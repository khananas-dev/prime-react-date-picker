...
export const Calendar = React.memo(
  React.forwardRef((props, ref) => {
    ...
    const onOverlayEnter = () => {
      if (props.autoZIndex) {
        ...
        ZIndexUtils.set(
          ...
          props.baseZIndex || PrimeReact.zIndex[key]
        );
      }
    ...
    const alignOverlay = () => {
      if (props.touchUI) {
        ...
        DomHandler.alignOverlay(
          overlayRef.current,
          inputRef.current,
          props.appendTo || PrimeReact.appendTo
        );
        ...
        if (appendDisabled()) {
          ...
          if (currentView === "date") {
            ...
            overlayRef.current.style.minWidth =
              DomHandler.getOuterWidth(inputRef.current) + "px";
          } else {
            overlayRef.current.style.width =
              DomHandler.getOuterWidth(inputRef.current) + "px";
          }
    ...
    const isNavIconClicked = (target) => {
      return (
        DomHandler.hasClass(target, "p-datepicker-prev") ||
        DomHandler.hasClass(target, "p-datepicker-prev-icon") ||
        DomHandler.hasClass(target, "p-datepicker-next") ||
    ...
    const isYearSelected = (year) => {
      if (isComparable()) {
        ...
        return !isMultipleSelection() && isComparable()
        ...
        try {
          if (isSingleSelection()) {
            ...
            for (let i = 0; i < value.length; i++) {
              ...
              let dateAsString = isValidDate(selectedValue)
